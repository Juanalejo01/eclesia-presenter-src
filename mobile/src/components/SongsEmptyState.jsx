 ...
 * SongsEmptyState
 *
 * Variantes idle | empty | empty-catalog | error | offline. Misma estetica
 * que BibleEmptyState (refactor potencial T13).
 */

const VARIANTS = {
  idle:           { icon: '🎵', tone: 'text-ink-3' },
  empty:          { icon: '🔎', tone: 'text-ink-3' },
  'empty-catalog':{ icon: '🎼', tone: 'text-ink-3' },
  error:          { icon: '⚠️', tone: 'text-live'  },
  offline:        { icon: '📡', tone: 'text-copper-200' },
}

export default function SongsEmptyState({
  variant = 'idle',
  message,
  hint,
  action,
